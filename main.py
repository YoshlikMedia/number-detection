...
def sort_cont(character_contours):
    """
    Chapdan o'ngga konturlarni saralash uchun
    """
    i = 0
...
def segment_chars(plate_img, fixed_width):
    """
    Raqamdagi belgilarni aniqlash uchun HSV formatidan qiymatlarni olib, 
    chegaralarni aniqlash kerak
    """
    ...
    # Raqamni o'lchamlarini o'zgartirish
    plate_img = imutils.resize(plate_img, width=fixed_width)
    thresh = imutils.resize(thresh, width=fixed_width)
    bgr_thresh = cv2.cvtColor(thresh, cv2.COLOR_GRAY2BGR)

    # Ulangan komponentlarni tahlil qilish va joylarini belgilash
    # Belgilarni turini aniqlash
    labels = measure.label(thresh, neighbors=8, background=0)

    charCandidates = np.zeros(thresh.shape, dtype='uint8')

    # yagona komponentlar bilan siklni aniqlash
    characters = []
    for label in np.unique(labels):
        # agar fon rasmi bo'lsa uni inobatga olmaydi
        if label == 0:
            continue
        # aks holda, ulangan komponentlarni ishga tushirish
        # konturlatni topish
        ...
        # agar hech bo'lmasa 1 ta kontur bo'lsa:
        if len(cnts) > 0:

            # Eng katta konturni topish
            # va uni chegarasini aniqlash
            c = max(cnts, key=cv2.contourArea)
            (boxX, boxY, boxW, boxH) = cv2.boundingRect(c)

            # olingan qismning nisbati, qattiqligi, balandligini aniqlash
            aspectRatio = boxW / float(boxH)
            solidity = cv2.contourArea(c) / float(boxW * boxH)
            heightRatio = boxH / float(plate_img.shape[0])

            # agar talablarga javob bersa
            # teslarni qo'llash
            ...
            if keepAspectRatio and keepSolidity and keepHeight and boxW > 14:
                # convexni hisoblash va uni chizish
    ...
    if contours:
        ...
        addPixel = 4  # belgining har bir o'lchamiga qo'shiladigan qiymat
        for c in contours:
...
class PlateFinder:
    def __init__(self):
        self.min_area = 4500  # minimum maydon
        self.max_area = 30000  # maximum maydon

        self.element_structure = cv2.getStructuringElement(shape=cv2.MORPH_RECT, ksize=(22, 3))

    def preprocess(self, input_img):
        imgBlurred = cv2.GaussianBlur(input_img, (7, 7), 0)  # eski sahifa
        gray = cv2.cvtColor(imgBlurred, cv2.COLOR_BGR2GRAY)  # kulrangga o'tkazish
        sobelx = cv2.Sobel(gray, cv2.CV_8U, 1, 0, ksize=3)  # sobelX - vertikal qirralar uchun
    ...
    def clean_plate(self, plate):
        ...
        if contours:
            ...
            max_index = np.argmax(areas)  # Eng katta kontur ko'rsatkichi
    ...
    def find_possible_plates(self, input_img):
        """
        Barcha konturlarni topish uchun
        """
        plates = []
        self.char_on_plate = []
...
class NeuralNetwork:
    ...
    def convert_tensor(self, image, imageSizeOuput):
        """
        tasvirni olib tenzorda o'zgartirish
        """
